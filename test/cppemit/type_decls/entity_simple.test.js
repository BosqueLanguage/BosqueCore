...
describe ("CPPEmit -- entity decl of number with invariants", () => {
    it("should emit entity with invariants", function () {
        ...
        checkTestEmitMainFunction('entity Foo { field f: Int; field g: Bool; invariant $g ==> $f > 3i; } public function main(): Foo { return Foo{1i, false}; }', "ccc");
    });
});
...
describe ("CPPEmit -- entity decl with functions", () => {
    it("should emit entity with functions", function () {
        ...
        checkTestEmitMainFunction('entity Foo<T> { function foo(x: T): T { return x; } } public function main(): Int { return Foo<Int>::foo(3i); }', "jjj");
        checkTestEmitMainFunction('entity Foo { function foo<T>(x: T): T { return x; } } public function main(): Int { return Foo::foo<Int>(3i); }', "lll");
    });
});
