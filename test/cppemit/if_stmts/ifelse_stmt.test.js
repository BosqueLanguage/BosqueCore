...
describe ("CPPEmit -- ElseIf Statement", () => {
    ...
    it("should emit if-else w/ single itest specials", function () {
        ...
        checkTestEmitMainFunction("public function main(): Int { let x: Option<Int> = some(3i); if (x)@some { return $x; } else { return 1i; } }", 'Int Mainᕒmain() { OptionᐸIntᐳ x = OptionᐸIntᐳ::fromSome(&ᐸRuntimeᐳ::g_typeinfo_SomeᐸIntᐳ, SomeᐸIntᐳ{3_i}); if(!x.isNone()) { Int ᑯx = x.unwrap(); return ᑯx; } else { None ᑯx = none; return 1_i; } }');
        checkTestEmitMainFunction("public function main(): Int { let x: Option<Int> = some(3i); if ($y = x)@!some { return 1i; } else { return $y; } }", 'Int Mainᕒmain() { OptionᐸIntᐳ x = OptionᐸIntᐳ::fromSome(&ᐸRuntimeᐳ::g_typeinfo_SomeᐸIntᐳ, SomeᐸIntᐳ{3_i}); if(x.isNone()) { None ᑯy = none; return 1_i; } else { Int ᑯy = x.unwrap(); return ᑯy; } }');
    ...
    it("should emit if-else w/ single itest types", function () {
        ...
        checkTestEmitMainFunction("public function main(): Int { let x: Option<Int> = some(3i); if (x)@<Some<Int>> { return $x.value; } else { return 1i; } }", 'Int Mainᕒmain() { OptionᐸIntᐳ x = OptionᐸIntᐳ::fromSome(&ᐸRuntimeᐳ::g_typeinfo_SomeᐸIntᐳ, SomeᐸIntᐳ{3_i}); if(!x.isNone()) { SomeᐸIntᐳ ᑯx = x.asSome(); return ᑯx.value; } else { None ᑯx = none; return 1_i; } }');
        checkTestEmitMainFunction("public function main(): Int { let x: Option<Int> = some(3i); if ($y = x)@<Some<Int>> { return $y.value; } else { return 1i; } }", 'Int Mainᕒmain() { OptionᐸIntᐳ x = OptionᐸIntᐳ::fromSome(&ᐸRuntimeᐳ::g_typeinfo_SomeᐸIntᐳ, SomeᐸIntᐳ{3_i}); if(!x.isNone()) { SomeᐸIntᐳ ᑯy = x.asSome(); return ᑯy.value; } else { None ᑯy = none; return 1_i; } }');
    });
