...
describe ("Checker -- If Statement", () => {
    ...
    it("should check binder & reflow itest ifs", function () {
        checkTestFunction("function main(): Int { var x: Option<Int> = some(3i); if(x)@@!some { return 0i; } return x; }");
        checkTestFunction("function main(): Int { var x: Option<Int> = some(3i); if(x)@@!some { return 0i; } return x; }");

        checkTestFunctionError("function main(): Int { let x: Option<Int> = some(3i); if(x)@@!some { return 0i; } return x; }", "Variable x is declared as const and cannot be re-typed");
    });
});
