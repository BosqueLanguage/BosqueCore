...
describe ("Checker -- Lambda calls (no template)", () => {
    it("should check lambda capture", function () {
        ...
        checkTestFunctionInFile('function foo(f: fn(Int) -> Int): Int { return f(1i); } function main(g: fn(Int) -> Int): Bool { let y = 1i; return foo(g); }');
        checkTestFunctionInFile('function foo(f: fn(Int) -> Int): Int { return f(1i); } function main(g: fn(Int) -> Int): Bool { let y = 1i; return foo(fn(x) => { return g(x) + y; }); }');
    });
});
