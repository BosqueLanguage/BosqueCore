...
describe ("Checker -- Simple if-expression", () => {
    ...
    it("should fail bad false", function () {
        checkTestExpError("if(1n < 2n) then 3i else none", "Int", "Expected a return value of type Int but got Int?");
    });

    it("should fail always true", function () {
        checkTestFunctionError("function main(x: Int): Int { return if(x)@none then 2i else 3i; }", "Test is never true -- true branch of if is unreachable");
    });

    it("should fail always false", function () {
        checkTestFunctionError("function main(x: Int): Int { return if(x)@some then 2i else 3i; }", "Test is never false -- false branch of if is unreachable");
    });
});