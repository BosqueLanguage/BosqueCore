...
describe ("flowit exec", () => {
    it("flowit should succeed", function () {
        runMainCode(`${flowit} public function main(): Nat { return flowit(none); }`, [0n, "Nat"]);
        runMainCode(`${flowit} public function main(): Nat { return flowit(some(5n)); }`, [15n, "Nat"]); 
    });
});

describe ("restrict exec", () => {
    it("restrict should succeed", function () {
        runMainCode(`${restrict} public function main(): Nat { return restrict(none); }`, [0n, "Nat"]);
        runMainCode(`${restrict} public function main(): Nat { return restrict(some(5n)); }`, [15n, "Nat"]); 
    });
});
