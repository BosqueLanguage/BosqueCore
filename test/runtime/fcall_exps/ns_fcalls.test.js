...
describe ("Exec -- NamespaceFunction (no template)", () => {
    ...
    it("should exec simple default", function () {
        runMainCode("function foo(x: Int, y: Int = $x): Int { return x + y; } public function main(): Int { return foo(1i, 2i); }", [3n, "Int"]);
        runMainCode("function foo(x: Int, y: Int = $x): Int { return x + y; } public function main(): Int { return foo(1i); }", [2n, "Int"]);
    });
});

describe ("Exec -- NamespaceFunction (with template)", () => {
    it("should exec simple positional", function () {
        runMainCode("function foo<T>(x: T): T { return x; } public function main(): Int { return foo<Int>(1i); }", [1n, "Int"]);
    });

    it("should exec two instantiations", function () {
        runMainCode("function foo<T>(x: T): T { return x; } public function main(): Int { if(foo<Nat>(1n) > 0n) { return foo<Int>(1i); } return 0i; }", [1n, "Int"]);
    });
});
