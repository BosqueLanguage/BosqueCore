...
describe ("Parser -- Lambda ref calls", () => {
    it("should parse simple ref full", function () {
        parseTestFunctionInFile('function foo(f: fn(inout Int) -> Int): Int { let x = 1i; return f(inout x); } [FUNC]', 'function main(): Bool { return foo(fn(inout x: Int): Int => x); }');
    ...
    it("should parse simple ref infer", function () {
        parseTestFunctionInFile('function foo(f: fn(inout Int) -> Int): Int { let x = 1i; return f(inout x); } [FUNC]', 'function main(): Bool { return foo(fn(inout x) => x); }');
