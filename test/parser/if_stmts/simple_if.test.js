...
describe ("Parser -- If Statement", () => {
    ...
    it("should parse binder & reflow itest ifs", function () {
        parseTestFunction("function main(): Int { var x: Option<Int> = some(3i); if(x)@@!some { return 0i; } return x; }", undefined);

        parseTestFunctionError("function main(): Int { var x: Option<Int> = some(3i); if($y = x)@@some { return $y; } return 1i; }", "Cannot have postflow without implicitdef or using a non-variable expression");
    });
});
