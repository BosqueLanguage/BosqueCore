...
describe ("Parser -- entity methods", () => {
    ...
    it("should parse simple entity methods with both template and more", function () {
        parseTestFunctionInFile('entity Foo<T> { field f: T; method foo<U>(t: T, u: U): U { if(this.f)@<U> { return $_; } else { return u; } }} [FUNC]', 'function main(): Bool { let x = Foo<Int>{3i}; return x.foo<Nat>(3n); }'); 
        parseTestFunctionInFile('entity Foo<T> { field f: T; method foo<U>(t: T, u: U): U { if(this.f)@<U> { return $_; } else { return u; } }} [FUNC]', 'function main(): Bool { let x = Foo<Int>{3i}; return x.foo<Int>(3i); }'); 
    });
...
describe ("Parser -- eADT methods", () => {
    ...
    it("should parse simple eADT methods with type template", function () {
        parseTestFunctionInFile('datatype Foo<T> of Foo1 { field f: T; method foo(x: T): T { if(true) { return x; } else { return this.f; } }} ; [FUNC]', 'function main(): Int { let x = Foo1<Int>{3i}; return x.foo(2i); }'); 
    });
});
