...
function buildAssembly(srcfile: string, stdlib: boolean): Assembly | undefined {
...
function emitCommandLineMakefile(): string {
    return 'MAKE_PATH=$(realpath $(dir $(lastword $(MAKEFILE_LIST))))\n' +
        ...
        `BUILD := debug\n\n` + 
        'CPP_STDFLAGS=-Wall -Wextra -Wno-unused-parameter -Wno-unused-variable -Wno-unused-but-set-variable -Wuninitialized -Werror -std=gnu++20 -fno-exceptions -fno-rtti -fno-strict-aliasing -fno-stack-protector -fPIC\n' + 
...
function runCPPBuild(code: string, nndir: string, stdlib: boolean): string | undefined {
...
function runTestSet(code: string, normalexecs: [string | undefined, string][], errorexecs: (string | undefined)[]): void {
    ...
    try {
        const emssg = runCPPBuild(code, nndir, false);
...
function runTestSet_stdlib(code: string, normalexecs: [string | undefined, string][], errorexecs: (string | undefined)[]): void {
    const nndir = fs.mkdtempSync(path.join(tmpdir(), "bosque-test-"));

    try {
        const emssg = runCPPBuild(code, nndir, true);
        assert.equal(emssg, undefined);
