...
namespace ᐸRuntimeᐳ
{
    ...
    public:
        ...
        constexpr static bool toBool(XBool b) { return (bool)b.value; }

        constexpr explicit operator bool() const { return this->value != 0ull; }

        friend constexpr XBool operator!(const XBool &b) { return XBool{(uint64_t)(!b.value)}; }
        friend constexpr XBool operator&(const XBool &lhs, const XBool &rhs) { return XBool{(uint64_t)(lhs.value & rhs.value)}; }
        friend constexpr XBool operator|(const XBool &lhs, const XBool &rhs) { return XBool{(uint64_t)(lhs.value | rhs.value)}; }
