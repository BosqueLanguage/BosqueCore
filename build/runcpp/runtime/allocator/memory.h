...
namespace ᐸRuntimeᐳ
{
    ...
    inline bool gcIsAllocated(const AtomicGCMetadata* rc) 
    {
        return (rc->load(std::memory_order_relaxed) & META_BIT_IS_ALLOC) != 0;
    }

    inline bool gcIsYoung(const AtomicGCMetadata* rc) 
    {
    ...
    inline void gcInitOnAllocate(AtomicGCMetadata* rc)
    {
    ...
    inline void gcProcessUpdateYoungForward(AtomicGCMetadata* rc)
    {
    ...
    inline bool gcProcessRCDecrement(AtomicGCMetadata* rc)
    {
        GCMetaBits ll = rc->load(std::memory_order_relaxed);
        while(true) {
            GCMetaBits newbits;
    ...
    inline void gcProcessSweep(AtomicGCMetadata* rc)
    {
