...
const smt_runtime_code_path = path.join(bosque_dir, "bin/mtruntime/formula.smt2");
...
function completeSMTComponent(cinfo: PendingCompnentInfo, err: any, result: string) {
    ...
    if(err) { 
        ...
        if(result === "unsat") {
			...
			let res = "";
			res = runSMTExtractor(asm!,cinfo.smtcomponents);
...
function checkSMTFormula(smtcomponents: string, outname: string): boolean {
    ...
    if(rr === "unsat") {
		...
		process.stdout.write(chalk.blue("\tExtracting Violation...") +"\n");
		let res = "";
		res = runSMTExtractor(asm!,outname)
		process.stdout.write(chalk.red("Violation Values:\n") + res + "\n");
...
function runSMTExtractor(assembly: Assembly,outdir: string): string {
