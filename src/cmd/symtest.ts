...
function completeSMTComponent(cinfo: PendingCompnentInfo, err: any, result: string) {
    ...
    if(err) { 
        ...
        if(result === "unsat") {
			...
			let res = "";
			const smtfile = path.join(cinfo.smt2file, "formula.smt2")
			res = runSMTExtractor(asm!,smtfile)
...
function checkSMTFormula(smtcomponents: string, outname: string): boolean {
    ...
    let rr = "";
    let start = Date.now();
	const smtfile = path.join(nndir, "formula.smt2")
    try {
        rr = execSync(`${z3bin} -T:60 ${smtfile}`).toString().trim();
    }
    catch(e) {
        Status.error("Failed to run SMT solver!\n");
        return false;
...
function runSMTExtractor(assembly: Assembly,smtfile: string): string {
	//Generate the json asminfo and fn_info
	//Get the directory of the files generated by it.

	const bsqfile = fullargs.find((v) => v.endsWith(".bsq"));

	let mock_fn = "";
	const fn_type = ischktest ? "chktest" : "errtest";
	for(let i = 0; i < assembly.toplevelNamespaces.length; ++i) {
		const nsdecl = assembly.toplevelNamespaces[i];

		for(let j = 0; j < nsdecl.functions.length; ++j){
			const fn_decl = nsdecl.functions[j];
			if(fn_decl.fkind == fn_type){
				mock_fn = fn_decl.name;
			}
		}
	}

	const mock_target = `--function  ${mock_fn}`;
	let json_dir = "";
    try {
