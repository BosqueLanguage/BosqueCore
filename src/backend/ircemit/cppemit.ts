...
class CPPEmitter {
    ...
    private isNominalTypeTask(t: IRTypeSignature): boolean {
        if(!(t instanceof IRNominalTypeSignature)) {
            return false;
        }

        return this.irasm.tasks.some((td) => td.tkey === t.tkeystr);
    }
    ...
    private emitIRSimpleExpression(exps: IRSimpleExpression, toplevel: boolean): string {
        if(exps instanceof IRImmediateExpression) {
            ...
            if(ttag === IRExpressionTag.IRAccessTypeDeclValueExpression) {
                ...
                bstr = `${TransformCPPNameManager.generateNameForConstructor(cexp.oftype.tkeystr)}{${this.emitIRSimpleExpression(cexp.key, false)}, ${this.emitIRSimpleExpression(cexp.value, false)}}`;
            }
                ...
                const mname = TransformCPPNameManager.convertIdentifier(afse.fieldname);
                if(this.isNominalTypeUniqueEntity(afse.eexptype) || this.isNominalTypeTask(afse.eexptype as IRNominalTypeSignature)) {
                    return `${this.emitIRSimpleExpression(afse.eexp, false)}${tinfo.getAccessor()}${mname}`;
                }
                else {
    ...
    private emitExpression(exp: IRExpression, toplevel: boolean): string {
        if(exp instanceof IRSimpleExpression) {
            ...
            if(ttag === IRExpressionTag.IRAccessEnvHasExpression) {
            ...
            else if(ttag === IRExpressionTag.IRConstructorListPassthroughExpression) {
                const ilpse = exp as IRConstructorListPassthroughExpression;
                return `${this.emitIRSimpleExpression(ilpse.ptl, false)}`;
            }
            ...
            else if(ttag === IRExpressionTag.IRConstructorListMixedExpression) {
                const iclme = exp as IRConstructorListMixedExpression;
                const cce = TransformCPPNameManager.convertTypeKey(iclme.constype.tkeystr);

                let chain = `${cce}{}`;
                for(let i = 0; i < iclme.elements.length; i++) {
                    const el = iclme.elements[i];
            ...
            else if(ttag === IRExpressionTag.IRConstructorMapEmptyExpression) {
                const icmee = exp as IRConstructorMapEmptyExpression;
                return `${TransformCPPNameManager.convertTypeKey(icmee.constype.tkeystr)}{}`;
            }
            else if(ttag === IRExpressionTag.IRConstructorMapSingletonsExpression) {
                const icmse = exp as IRConstructorMapSingletonsExpression;
                const cce = TransformCPPNameManager.convertTypeKey(icmse.constype.tkeystr);
                const args = icmse.elements.map((vv) => this.emitIRSimpleExpression(vv, true)).join(", ");
                ...
                const ifcsexp = exp as IRInterpolateFormatStringExpression;
                const icfs = this.emitIRSimpleExpression(ifcsexp.fmtString, true);
                const argstrs = ifcsexp.args.map((arg) => this.emitIRSimpleExpression(arg, true));
                return `${RUNTIME_NAMESPACE}::XFString::interpolate<${argstrs.length}>(${icfs}.fcid, { ${argstrs.join(", ")} })`;
            }
            else {
    ...
    private emitAtomicStatement(stmt: IRAtomicStatement): string {
        ...
        if(ttag === IRStatementTag.IRNopStatement) {
        ...
        else if(ttag === IRStatementTag.IRUpdateLocalDirectStatement) {
            const uldstmt = stmt as IRUpdateLocalDirectStatement;
            const tlinfo = this.typeInfoManager.getTypeInfo(uldstmt.vtype.tkeystr);
            
            const aargs = uldstmt.aargs.map((arg) => this.emitIRSimpleExpression(arg, true));
            if(tlinfo.tag === LayoutTag.Value) {
                return `${TransformCPPNameManager.convertIdentifier(uldstmt.vname)} = {${aargs.join(", ")}};`;
            }
            else {
                const cce = TransformCPPNameManager.generateNameForConstructor(uldstmt.vtype.tkeystr);
                return `${TransformCPPNameManager.convertIdentifier(uldstmt.vname)} = ᐸRuntimeᐳ::${cce}_allocator.allocate(${aargs.join(", ")});`;
            }
        }
        else if(ttag === IRStatementTag.IRUpdateParamDirectStatement) {
            const updstmt = stmt as IRUpdateParamDirectStatement;
            const tlinfo = this.typeInfoManager.getTypeInfo(updstmt.vtype.tkeystr);
            
            const aargs = updstmt.aargs.map((arg) => this.emitIRSimpleExpression(arg, true));
            if(tlinfo.tag === LayoutTag.Value) {
                return `${TransformCPPNameManager.convertIdentifier(updstmt.vname)} = {${aargs.join(", ")}};`;
            }
            else {
                const cce = TransformCPPNameManager.generateNameForConstructor(updstmt.vtype.tkeystr);
                return `${TransformCPPNameManager.convertIdentifier(updstmt.vname)} = ᐸRuntimeᐳ::${cce}_allocator.allocate(${aargs.join(", ")});`;
            }
        }
            ...
            const tinfoname = `ᐸRuntimeᐳ::g_typeinfo_${TransformCPPNameManager.convertTypeKey(ids.oftype.tkeystr)}`;
            return `{ auto __val = ${this.emitIRSimpleExpression(ids.dbgexp, true)}; std::cout << "**DIAGNOSTIC**" << std::endl; ${tinfoname}.opdispatch.displayFp(&${tinfoname}, &__val, std::cout, std::nullopt); std::cout << std::endl; }`;
        }
        else {
    ...
    */
    ...
    private emitBuiltinBody(invk: IRInvokeDecl, body: IRBuiltinBody, indent: string | undefined): string {
        ...
        if(body.builtin === "bool_to_cstring") {
            ...
            const rtype = this.typeInfoManager.getTypeInfo(invk.resultType.tkeystr);
            bstr = `ᐸRuntimeᐳ::XListOps::fromRange<Int, ${rtype.bsqtypeid}>(start.value, end.value, step.value, (bool)inclusive.value)`;
        }
    ...
    private emitIRInvokeParameterDecl(iparam: IRInvokeParameterDecl): string {
        ...
        if(iparam.skind === undefined) {
            ...
            if(iparam.skind === "this" || iparam.skind === "self") {
                return `${ptypstr} ${TransformCPPNameManager.convertIdentifier(iparam.name)}`;
            }
    ...
    private emitIRTaskActionDeclInfo(itaskaction: IRTaskActionDecl): [string, string] {
        const haspassing = itaskaction.params.some((p) => p.pkind !== undefined);

        const precondinfo = itaskaction.preconditions.map((ipcs) => this.emitPreconditionCheckFunction(ipcs, itaskaction));
        const postcondinfo = itaskaction.postconditions.map((ipcs) => this.emitPostconditionCheckFunction(ipcs, itaskaction, haspassing));

        const ppdecls = [...precondinfo, ...postcondinfo].map((ppc) => ppc[0]);
        const ppdefs = [...precondinfo, ...postcondinfo].map((ppc) => ppc[1]);

        const paramstrs = itaskaction.params.map((param) => this.emitIRInvokeParameterDecl(param)).join(", ");
        const rettyps = this.typeInfoManager.emitTypeAsReturn(itaskaction.resultType.tkeystr);

        const bodystr = this.emitBody(itaskaction, "");
        
        const ideclstr = `${rettyps} ${TransformCPPNameManager.convertInvokeKey(itaskaction.ikey)}(${paramstrs});`;
        const idefstr = `${rettyps} ${TransformCPPNameManager.convertInvokeKey(itaskaction.ikey)}(${paramstrs}) ${bodystr}`;
    ...
    private emitEnumTypeInfoDecl(tdecl: IREnumTypeDecl): string {
        ...
        `        nullptr,\n` +
        `        0,` +
        `        TypeOpDispatchInfo{ (ValidatingConstructorFp)validatingConstructor_${ctname}, (JSONParseToBSQFp)&jsonParseToBSQ_Enum, (ParseToBSQFp)&parseToBSQ_Enum, (BSQToJSONFp)&bsqToJSON_Enum, (BSQToBAPIFp)&bsqToBAPI_Enum, (DisplayValueFp)&displayValue_Enum },\n` +
        `        "${tdecl.tkey}",\n` +
        `        true\n` +
        `    };\n` +
        `}`;
    ...
    private emitListTypeInfoDecl(tdecl: IRListTypeDecl): [string, string] {
        ...
        const ftdecl = `    inline constexpr TypeLayoutInfo g_ftable_${ctname}[1] = { { -1, ${ofttid.bsqtypeid}, 0, 0, "value", "value" } };\n`;
        ...
        ftdecl +
        '\n' +
        `    inline constexpr TypeInfo g_typeinfo_PosRBTreeLeaf_${ctname} = g_typeinfo_PosRBTreeLeaf_generate<${oftrepr}, ListTTreeContent<${oftrepr}, ${ttid.bsqtypeid}>::MAX_LEAF_CAPACITY>(${posrb_treeleafid}, ${leafmask !== undefined ? `"${leafmask}"` : "nullptr"}, "PosRBTreeLeaf_${tdecl.tkey}", ${ofttid.quickrelease});\n` +
        `    inline constexpr TypeInfo g_typeinfo_PosRBTreeNode_${ctname} = g_typeinfo_PosRBTreeNode_generate<${oftrepr}, ListTTreeContent<${oftrepr}, ${ttid.bsqtypeid}>::MAX_LEAF_CAPACITY>(${posrb_treenodeid}, "${nodemask}", "PosRBTreeNode_${tdecl.tkey}");\n` +
        `    inline constexpr TypeInfo g_typeinfo_PosRBTree_${ctname} = g_typeinfo_PosRBTree_generate<${oftrepr}, ListTTreeContent<${oftrepr}, ${ttid.bsqtypeid}>::MAX_LEAF_CAPACITY, ${ttid.bsqtypeid}>(${posrb_treeid}, "PosRBTree_${tdecl.tkey}");\n` +
        '\n' +
        ...
        '\n' +
        `    inline constexpr TypeInfo g_typeinfo_${ctname}Inline = g_typeinfo_ListTInlineContent_generate<${oftrepr}>(${listinlineid}, ${inlinemask !== undefined ? `"${inlinemask}"` : "nullptr"}, "${tdecl.tkey}Inline");\n` +
        `    inline constexpr TypeInfo g_typeinfo_${ctname}Tree = g_typeinfo_ListTTreeContent<${oftrepr}, ${ttid.bsqtypeid}>(${listtreeid}, "${tdecl.tkey}TreeContent");\n` +
        `    inline constexpr TypeInfo g_typeinfo_${ctname} = g_typeinfo_ListT_generate<${oftrepr}, ${ttid.bsqtypeid}>(${ttid.bsqtypeid}, g_ftable_${ctname}, ${inlinemask !== undefined ? `"${inlinemask}"` : "nullptr"}, "${tdecl.tkey}");\n` +
        `}`;
    ...
    private emitMapTypeInfoDecl(tdecl: IRMapTypeDecl): [string, string] {
        ...
        const cmprb_treeleafid = ttid.bsqtypeid - 3;
        const cmprb_treenodeid = ttid.bsqtypeid - 2;
    ...
    private emitEntityTypeInfoWForwarDecl(tdecl: IRAbstractEntityTypeDecl): string {
        ...
        const vconsdecl = `    void validatingConstructor_${ctname}(void** argptrs, void* resptr);\n`;
    ...
    private emitEnumTypeDeclInfo(eenum: IREnumTypeDecl): [string, string] {
        ...
        `    friend Bool operator>=(const ${ctname}& lhs, const ${ctname}& rhs) { return ᐸRuntimeᐳ::XBool::from(!(lhs.value < rhs.value)); }\n` +
        '\n' +
        `    constexpr static const char* BSQ_members[${eenum.members.length}] = { ${eenum.members.map((mem) => `"${mem}"`).join(", ")} };\n` +
        `};`;

        const consdecl = `inline void validatingConstructor_${ctname}(void** args, void* trgt) { *(${ctname}*)trgt = ${ctname}{*((uint64_t*)args[0])}; }\n`;

        const mdecls = `${eenum.members.map((mem, ii) => `${ctname} ${ctname}::${TransformCPPNameManager.convertIdentifier(mem)} = ${ctname}{${ii}};`).join("\n")}\n`;

        return [
            [edecl, consdecl, this.emitEnumTypeInfoDecl(eenum)].join("\n"), 
            [mdecls].join("\n")
        ];
    }
    ...
    private emitIRAPIErrorTypeInfo(tdecl: IRAPIErrorTypeDecl): [string, string] {
        return this.emitIRAPIEntityGeneral(tdecl, tdecl.ttype, "Error");
    }

    private emitIRAPIRejectedTypeInfo(tdecl: IRAPIRejectedTypeDecl): [string, string] {
        return this.emitIRAPIEntityGeneral(tdecl, tdecl.ttype, "Rejected");
    }

    private emitIRAPIDeniedTypeInfo(tdecl: IRAPIDeniedTypeDecl): [string, string] {
        return this.emitIRAPIEntityGeneral(tdecl, tdecl.ttype, "Denied");
    }

    private emitIRAPIDroppedTypeInfo(tdecl: IRAPIDroppedTypeDecl): [string, string] {
        return this.emitIRAPIEntityGeneral(tdecl, tdecl.ttype, "Dropped");
    }

    private emitIRAPISuccessTypeInfo(tdecl: IRAPISuccessTypeDecl): [string, string] {
        return this.emitIRAPIEntityGeneral(tdecl, tdecl.ttype, "Success");
    }
    ...
    private emitMapTypeInfo(tdecl: IRMapTypeDecl): [string, string] {
        const ctname = TransformCPPNameManager.convertTypeKey(tdecl.tkey);
        const tinfo = this.typeInfoManager.getTypeInfo(tdecl.tkey);
    ...
    private emitAPIResultTypeInfo(tdecl: IRAPIResultTypeDecl): [string, string] {
        const ctname = TransformCPPNameManager.convertTypeKey(tdecl.tkey);
        const uctname = TransformCPPNameManager.generateNameForUnionType(tdecl.tkey);
        
        const uoptions = [tdecl.errortype, tdecl.rejectedtype, tdecl.deniedtype, tdecl.droppedtype, tdecl.successtype];
    ...
    private emitIRTaskDeclInfo(tdecl: IRTaskDecl): [string, string] {
        const ctname = TransformCPPNameManager.convertTypeKey(tdecl.tkey);
        
        const iifieldargl = tdecl.fields.map((bf) => { return {pname: `${TransformCPPNameManager.convertIdentifier("$" + bf.fname)}`, ptype: bf.declaredType}; }); 
        const vfuncinfo = tdecl.invariants.map((inv) => this.emitInvariantFunction(inv, tdecl, iifieldargl));
        const valfuncinfo = tdecl.validates.map((val) => this.emitValidateFunction(val, tdecl, iifieldargl));

        const fdecllist = tdecl.fields.map((bf) => {
            const ftypstr = this.typeInfoManager.emitTypeAsMemberField(bf.declaredType.tkeystr);
            const fname = TransformCPPNameManager.convertIdentifier(bf.fname);
            return `    ${ftypstr} ${fname};`;
        });
        const tclass = `class ${ctname} {\n` +
            `public:\n` +
            (tdecl.fields.length !== 0 ? `${fdecllist.join("    \n")}\n` : "    uint64_t padding; // Special case: no fields\n") +
            `    //All constructor and assignment defaults\n` +
            `};`;
    ...
    private emitTypeDeclInfo(): [string, string] {
        ...
        .map((ttd) => {
            if(!(ttd instanceof IRNominalTypeSignature)) {
                ...
                if (ctd instanceof IRConstructableTypeDecl) {
                    ...
                    else if(ctd instanceof IRAPIErrorTypeDecl) {
                        return this.emitIRAPIErrorTypeInfo(ctd);
                    }
                    else if(ctd instanceof IRAPIRejectedTypeDecl) {
                        return this.emitIRAPIRejectedTypeInfo(ctd);
                    }
                    else if(ctd instanceof IRAPIDeniedTypeDecl) {
                        return this.emitIRAPIDeniedTypeInfo(ctd);
                    }
                    else if(ctd instanceof IRAPIDroppedTypeDecl) {
                        return this.emitIRAPIDroppedTypeInfo(ctd);
                    }
                    else if(ctd instanceof IRAPISuccessTypeDecl) {
                        return this.emitIRAPISuccessTypeInfo(ctd);
                    }
                    else if (ctd instanceof IRMapEntryTypeDecl) {
                        return this.emitMapEntryTypeInfo(ctd);
                    }
                    else {
                    ...
                    if(ctd instanceof IRListTypeDecl) {
                        ...
                        return this.emitMapTypeInfo(ctd);
                    }
                    else {
                    ...
                    else if(ctd instanceof IRAPIResultTypeDecl) {
                        return this.emitAPIResultTypeInfo(ctd);
                    }
                    else {
        ...
        const tasks = this.irasm.tasks.map((task) => this.emitIRTaskDeclInfo(task));
    ...
    private emitAllInvokeInfo(): [string, string] {
        ...
        const taskdecls = this.irasm.taskactions.map((task) => this.emitIRTaskActionDeclInfo(task));

        const constdecls = this.irasm.constants.map((c) => this.emitConstantDeclInfo(c));

        return [
            [...idecls.map((idecl) => idecl[0]), ...taskdecls.map((tdecl) => tdecl[0]), ...constdecls.map((cdecl) => cdecl[0])].join("\n\n"),
            [...idecls.map((idecl) => idecl[1]), ...taskdecls.map((tdecl) => tdecl[1]), ...constdecls.map((cdecl) => cdecl[1])].join("\n\n")
        ];
    }
    ...
    private emitStaticInitializationOps(): string {
        ...
        const enuminfoentries = this.irasm.enums.map((e) => {
            const ctname = TransformCPPNameManager.convertTypeKey(e.tkey);
            const tinfo = this.typeInfoManager.getTypeInfo(e.tkey); 
            return `{ ${tinfo.bsqtypeid}, { ${e.members.length}, { (const char**)${ctname}::BSQ_members } } }`; 
        });
    ...
    private emitParseArgsMain(params: IRInvokeParameterDecl[]): string {
        if(params.length === 0) {
            return '    //No args';
        }
        else {
    ...
    private emitTMain(tdecl: IRTaskDecl): string {
    ...
    private emitCommandLineMain(ns: string): string | undefined {
        const idecl = this.irasm.invokes.find((v) => v.ikey === `${ns}::main`);
        const tdecl = this.irasm.tasks.find((v) => v.tkey === `${ns}::Main`);
        if(idecl === undefined && tdecl === undefined) {
            return undefined;
        }
        
        const mmain = (idecl !== undefined) ? this.emitFMain(idecl) : this.emitTMain(tdecl as IRTaskDecl);
    ...
    public emitForCommandLine(ns: string): [string, string] | undefined {
        ...
        const mainstr = this.emitCommandLineMain(ns);
        if(mainstr === undefined) {
            return undefined;
        }
