...
namespace BSQON
{
    ...
    {
        assert(!this->m_recoverStack.empty());

        auto tks = this->m_recoverStack.back();
        std::sort(tks.begin(), tks.end());
        while(!this->m_lex.testToken(TokenKind::TOKEN_EOF) && !std::binary_search(tks.begin(), tks.end(), this->m_lex.peekToken().kind)) {
            this->m_lex.popToken();
        }
