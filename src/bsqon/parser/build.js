...
exec(`bison -d${mode === "debug" ? " -Wcex" : ""} bsqon.y && flex bsqon.l`, {cwd: srcdir}, (err, stdout, stderr) => {
    ...
    exec(bsqon_obj_files.map((v) => `gcc -O0 -ggdb -DEXPORT -o ${v[1]} -c ${v[0]}`).join(" && "), {cwd: srcdir}, (oerr, ostdout, ostderr) => {
        ...
        exec(`g++ -O0 -ggdb ${cpp_flags} ${json_includes} -o ${outexec}/bsqon ${bsqon_cpp_files.join(" ")} ${bsqon_obj_files.map((v) => v[1]).join(" ")} bsqon_main.cpp -lgmp`, {cwd: bsqonsrcdir}, (berr, bstdout, bstderr) => {
            donebsqon = true;
