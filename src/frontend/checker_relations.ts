...
import { AbstractConceptTypeDecl, AdditionalTypeDeclTag, Assembly, CCharBufferTypeDecl, ConceptTypeDecl, ConstMemberDecl, DatatypeMemberEntityTypeDecl, DatatypeTypeDecl, EntityTypeDecl, FailTypeDecl, InternalEntityTypeDecl, MemberFieldDecl, MethodDecl, OkTypeDecl, OptionTypeDecl, PrimitiveEntityTypeDecl, ResultTypeDecl, SomeTypeDecl, TaskDecl, TemplateTermDeclExtraTag, TypeFunctionDecl, TypedeclTypeDecl, MapEntryTypeDecl, AbstractEntityTypeDecl, ValidateDecl, InvariantDecl, AbstractCollectionTypeDecl, ListTypeDecl, StackTypeDecl, QueueTypeDecl, SetTypeDecl, MapTypeDecl, NamespaceDeclaration, EnumTypeDecl, UnicodeCharBufferTypeDecl } from "./assembly.js";
...
class TypeCheckerRelations {
    ...
    //Check if this type is a primitive type in Core
    isEnumType(t: TypeSignature): boolean {
        assert(!(t instanceof ErrorTypeSignature), "Checking primitive on errors");
    ...
    getExpandoableOfType(t: TypeSignature): TypeSignature | undefined {
        ...
        else if(decl instanceof CCharBufferTypeDecl || decl instanceof UnicodeCharBufferTypeDecl) { // Only possible type is CChar or UnicodeChar
            return t;
        }
        else {
    ...
    resolveAllTypeDeclaredValidatorDecls(ttype: TypeSignature, tconstrain: TemplateConstraintScope): {invariants: MemberLookupInfo<InvariantDecl>[], validators: MemberLookupInfo<ValidateDecl>[]} {
        const allinvariants = ((ttype as NominalTypeSignature).decl.invariants.map((inv) => new MemberLookupInfo<InvariantDecl>(new TypeLookupInfo(ttype as NominalTypeSignature, TemplateNameMapper.createEmpty()), inv)));
        const allvalidators = ((ttype as NominalTypeSignature).decl.validates.map((inv) => new MemberLookupInfo<ValidateDecl>(new TypeLookupInfo(ttype as NominalTypeSignature, TemplateNameMapper.createEmpty()), inv)));
