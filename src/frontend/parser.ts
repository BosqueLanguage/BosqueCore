...
class Lexer {
    ...
    private static computeBytesPerChar(cc: string): number {
        //this is pretty brutal perf wise but we want to be simple and obvious for now

        return Buffer.from(cc, "utf8").length;
    }

    private advancePositionInfo(epos: number): [number, number] {
        const u16count = epos - this.jsStrPos;
        let u8count = 0;
        for(let i = this.jsStrPos; i < epos ; ++i) {
            u8count += Lexer.computeBytesPerChar(this.input[i]);
        }
    ...
    constructor(iscore: boolean, srcfile: string, input: string, macrodefs: string[]) {
        ...
        const [u16count, u8count] = this.advancePositionInfo(this.input.length);
        this.jsStrEnd = u16count;
        this.utf8StrEnd = u8count;
    ...
    private tryLexLineComment(): boolean {
        ...
        let jepos = this.input.slice(0, this.jsStrEnd).indexOf("\n", this.jsStrPos);
    ...
    private tryLexDocComment(): boolean {
        ...
        let jepos = this.input.slice(0, this.jsStrEnd).indexOf(" **%", this.jsStrPos + 4);
        if (jepos !== -1) {
            jepos += 4;
    ...
    private tryLexSpanComment(): boolean {
        ...
        let jepos = this.input.slice(0, this.jsStrEnd).indexOf("*%", this.jsStrPos + 2);
        if (jepos === -1) {
...
class Parser {
    ...
    private parseSamples(sinfo: SourceInfo, boundtemplates: Set<string>): InvokeExample[] {
        ...
        while (this.testToken(KW_spec) || this.testToken(KW_test) || this.testToken(KW_example)) {
            let ekind: InvokeExampleKind | undefined = undefined;
            while(this.testToken(KW_spec) || this.testToken(KW_test) || this.testToken(KW_example)) {
                if(ekind !== undefined) {
                ...
                if(this.testToken(KW_spec)) {
                    ...
                    ekind = InvokeExampleKind.Std;
                }
                this.consumeToken();
            }
            ekind = ekind || InvokeExampleKind.Std;
    ...
    */
    ...
    private parseEntityRegisterType(sinfo: SourceInfo, attributes: DeclarationAttibute[], name: string, etag: AdditionalTypeDeclTag) {
        ...
        if(this.env.currentNamespace.fullnamespace.ns[0] === "Core") {
            if(PRIMITIVE_ENTITY_TYPE_NAMES.includes(name)) {
                ...
                tdecl = new PairTypeDecl(this.env.currentFile, sinfo, attributes, "Pair");
            }
            else if(name === "Some") {
    ...
    private parseEntityCompleteParse(sinfo: SourceInfo, name: string) {
        ...
        else if(tdecl instanceof PairTypeDecl) {
            this.parseOOPMembersCommonAll(false, undefined, new Set<string>(["T", "U"]), undefined, undefined, tdecl.consts, tdecl.functions, undefined, tdecl.methods, undefined, undefined, undefined);
        }
    ...
    private parseConceptCompleteParse(sinfo: SourceInfo, name: string) {
        ...
        if(tdecl instanceof PrimitiveConceptTypeDecl) {
            ...
            this.parseOOPMembersCommonAll(false, undefined, new Set<string>(["T", "E"]), undefined, undefined, tdecl.consts, tdecl.functions, undefined, tdecl.methods, undefined, undefined, undefined);
        }
    ...
    private parseEnum(attributes: DeclarationAttibute[], endtok: string) {
        ...
        if(isParsePhase_Enabled(this.currentPhase, ParsePhase_RegisterNames)) {
            ...
            tdecl.provides.push(this.wellknownTypes.get("Any") as TypeSignature);
